...
class SetCriterion(nn.Module):
    ...
    def loss_boxes(self, outputs, targets, indices, num_masks):
        ...
        # Stack sizes and swap H,W to W,H for box normalization
        sizes = torch.stack([t["orig_size"] for t in targets])  # [batch_size, 2] with [H, W]
        sizes = sizes[:, [1, 0]]  # Swap to [W, H]
        # Index with batch_idx and repeat for [W, H, W, H]
        target_sizes = torch.cat([sizes[tgt_idx[0]], sizes[tgt_idx[0]]], dim=1)  # [num_targets, 4]
        target_boxes = target_boxes / target_sizes
